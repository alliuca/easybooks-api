...
module.exports = (doc, data) => {
  ...
  doc.rect(0, 0, page.width, 120).fill(primaryColor);
  ...
  doc
    ...
    .fill("white")
    .image(
      `${process.cwd()}/public/files/upload/logo/${data.settings.logo}`,
      40,
      42,
      { fit: [40, 40] }
    );
  ...
  doc
    .fontSize(fontSizeBody)
    .fillColor("white")
    .text(data.addressStreet, page.width - 220, 40, {
      width: 180,
      align: "right"
    });
  ...
  doc
    .fontSize(fontSizeBody)
    .fillColor("white")
    .text(data.postalCode, page.width - 220, 80, {
      width: 180,
      align: "right"
    });
  ...
  doc
    ...
    .text("Intestazione Fattura", 35, 150);
  ...
  doc
    .fontSize(fontSizeBody)
    .fillColor(secondaryColor)
    .text("Data di emissione", 210, 208);
  ...
  doc
    .fontSize(fontSizeBody)
    .fillColor(secondaryColor)
    .text("Totale Fattura", page.width - (200 + paddingHorizontal), 150, {
      width: 200,
      align: "right"
    });
  ...
  doc
    ...
    .text(
      utils.formatCurrency(data.currency.value, data.amount),
      page.width - (200 + paddingHorizontal),
      170,
  ...
  doc
    .fontSize(fontSizeTableHeading)
    .fillColor(primaryColor)
    .text("Descrizione", 35, 305);

  doc
    .fontSize(fontSizeTableHeading)
    .fillColor(primaryColor)
    .text("Ore", 440, 305, { width: 60, align: "right" });

  doc
    .fontSize(fontSizeTableHeading)
    .fillColor(primaryColor)
    .text("Importo", page.width - (70 + paddingHorizontal), 305, {
      width: 70,
      align: "right"
    });
  ...
  while (i < data.items.length) {
    ...
    doc
      .fontSize(fontSizeBody)
      .fillColor("black")
      .text(data.items[i].hours, 440, 340 + i * spacingY, {
        width: 60,
        align: "right"
      });
    ...
    doc
      ...
      .fillColor("black")
      .text(
        utils.formatCurrency(data.currency.value, data.items[i].amount),
        page.width - (70 + paddingHorizontal),
        340 + i * spacingY,
        { width: 70, align: "right" }
      );

    doc
      .moveTo(30, 375 + i * spacingY)
  ...
  doc
    .fontSize(fontSizeBody)
    .fillColor(secondaryColor)
    .text("Termini Fattura", 35, 515, { width: 200 });
  ...
  data.fees.items.map(item => {
    ...
    fees.values += `${utils.formatCurrency(data.currency.value, item.value)}\n`;
  });
  ...
  doc
    ...
    .fillColor("black")
    .text(
      `${utils.formatCurrency(data.currency.value, data.subtotal)}\n${
        fees.values
      }${utils.formatCurrency(
        data.currency.value,
        data.amount
      )}\n\n${utils.formatCurrency(data.currency.value, data.amount)}`,
      page.width - (110 + paddingHorizontal),
      515,
      { width: 110, align: "right", lineGap: 13.8 }
    );
